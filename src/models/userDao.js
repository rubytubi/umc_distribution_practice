...
import {confirmEmail, connectFoodCategory, getPreferToUserID, getUserID, insertUserSql} from './userSQL.js';
...
export const addUser =
  ...
  try {
    ...
    const result = await pool.query(insertUserSql, [
      data.email, data.name, data.gender, data.birth, data.addr, data.specAddr,
      data.phone
    ]);
...
// 음식 선호 카테고리 매핑
export const setPrefer =
    async (userId, foodCategoryId) => {
  try {
    const conn = await pool.getConnection();
